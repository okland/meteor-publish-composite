Publication = function Publication(subscription, options, args) {
    ...
    if (options.attachParent && args && args.length) {
       this.parentId = args[0] && args[0]._id;
    }
    ...
    this.observeHandle = this.cursor.observe({
        added: function added(doc) {
            ...
            if (alreadyPublished) {
                ...
                self.subscription.added(collectionName, doc, self.parentId);
...
Publication.prototype._removeDoc = function _removeDoc(collectionName, docId) {
    this.subscription.removed(collectionName, docId, this.parentId);
